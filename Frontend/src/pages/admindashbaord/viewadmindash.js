...
const AdminDashboard = () => {
    ...
    const handleUpdate = async (values) => {
        try {
            const response = await axios.put(`/update-details`, {
    ...
    const handleDelete = async (email) => {
        try {
            const response = await axios.delete(`/authUser/deleteUser/${email}`);
            if (response.data.success) {
        ...
        },
        {
            ...
            render: (_, record) => (
                <>
                    ...
                    <Button icon={<DeleteOutlined />} onClick={() => handleDelete(record.email)} danger style={{ marginLeft: 8 }} />
                </>
            ),
        },
    ];
